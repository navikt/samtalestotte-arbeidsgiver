export const cookieInitializer = (cookies: { [name: string]: any }): CookieState => {
    ...
    if (cookies !== undefined) {
        ...
        sendtStatistikk =
            cookies['sendtStatistikk'] === undefined ? 'nei' : cookies['sendtStatistikk'];
    }
...
export type OppfølgingSamtaleState = Record<
    typeof oppfølgningsSamtaleKeys[number],
    string | undefined
>;

const samtaleverktøyKeys = [
...
export interface CookieReducerAction {
    type:
        | keyof SituasjonQAState
        | keyof SamtaleverktøyState
        | keyof OppfølgingSamtaleState
        | 'sendtStatistikk';
...
const oppfølgingSamtaleReducer = (
    state: Partial<OppfølgingSamtaleState>,
    action: CookieReducerAction
): Partial<OppfølgingSamtaleState> => {
...
const samtaleverktøyReducer = (
    state: Partial<SamtaleverktøyState>,
    action: CookieReducerAction
): Partial<SamtaleverktøyState> => {
...
const situasjonQAReducer = (
    state: Partial<SituasjonQAState>,
    action: CookieReducerAction
): Partial<SituasjonQAState> => {
