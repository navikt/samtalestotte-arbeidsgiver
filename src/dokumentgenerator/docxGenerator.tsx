...
const mapJson = (elements: (string | object)[]): (DocxTypes)[] => {
    return elements.map(e => {
        ...
        return undefined
    }).flat().filter(notUndefinedOrNull)
}
...
const mapColumns = (
    leftContent: DocumentElement[],
    rightContent: DocumentElement[]
) => {
    return new Table({
...
const mapTableCell = (content : DocumentElement[], shade: string="#ffffff" ) => {
    ...
    }).filter(notUndefinedOrNull).flat()
...
const mapLink = (text: string, url: string) => {
    return url.startsWith("#")
        ? mapInternalHyperLink(text, url)
        : mapExternalHyperLink(text, url)
}
...
const mapList = (content: (DocumentElement | string)[][], level: number = 0): Paragraph[] => {
    return content.flatMap( x =>
        x.flatMap((e, i) => {
            ...
            if(isParagraph(e)){return mapParagraph(e.content, level)}
            if(isLink(e)){
                return i == 0
                    ? new Paragraph({
...
const mapPanel = (title: string, content: (DocumentElement | string)[], id?: string) => {
    return [
        new Paragraph({
            ...
            children: [
                new Bookmark({
                    id: notUndefinedOrNull(id)? id : `ekspanderbart-infopanel__${camelCase(title)}-base`,
                    children: [
                        new TextRun(title),
                    ],
                }),
...
const indentAll = (content: (DocumentElement | string)[] | string, level: number) => {
    ...
    return content.map(element => {
        ...
        if(isText(element)){
            return {...element, content: `${"\t".repeat(level+1)}${element.content}`} as DocumentElement
        }
        if(isLink(element)){
            return {...element, content: `${"\t".repeat(level+1)}${element.content}`} as DocumentElement
        }
